...
import { WorkflowLayoutDirection } from "diagram-maker";
import {
  DiagramMakerNode,
...
export function createPotentialNode(
...
export function createRectangularNode(
  node: DiagramMakerNode<{ odd?: boolean }>,
  container: HTMLElement
) {
...
export function createCircularNode(
  node: DiagramMakerNode<any>,
  container: HTMLElement
) {
  const id = node.id.substring(0, 13);
...
export function createLibraryPanel(container: HTMLElement) {
  ...
  newDiv.appendChild(
    createPanelNode("testId-normalWithSize", "Normal with Size", {
      width: 100,
      height: 50,
    })
  );
...
function createHierarchicalLayoutButton(
  getDiagramMakerObj: () => DiagramMaker
) {
...
export function createPluginPanel(container: HTMLElement, state: any) {
  ...
  const testPlugin = createDivWithText(
    "Click this plugin to move workspace to a target position"
  );
...
export function updateActionInLogger(action: Action) {
  ...
  if (logger) {
    ...
    if (anyAction.payload) {
      const payload = createDivWithText(
        `Payload is ${JSON.stringify(anyAction.payload)}`
      );
